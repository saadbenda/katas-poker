...
public class DoublePaireSolver {
	...
	public static Score calcul(Main main) {
		...
		List<List<Carte>> filtre = groupe.values().stream().filter(l -> l.size() == 2).collect(Collectors.toList());
		
		if (filtre.size() == 2) {
			return new Score(Combinaison.DOUBLE_PAIRE, filtre.get(0).get(0));
		}
